...
const Header = ({ fontsLoaded }: { fontsLoaded: boolean }) => {
  ...
  // Optimized scroll handler without delay
  useEffect(() => {
    let lastKnownScrollY = 0;
    let ticking = false;
    let animationState = window.scrollY >= 150 ? "small" : "full";
    ...
    const handleScroll = () => {
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          // Check if we've crossed the threshold and need to change state
          const newState = lastKnownScrollY >= 150 ? "small" : "full";
