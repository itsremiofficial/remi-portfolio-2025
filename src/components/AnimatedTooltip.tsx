...
const AnimatedTooltip = ({
  ...
  useEffect(() => {
    if (shouldRenderTooltip && tooltipRef.current) {
      gsap.fromTo(tooltipRef.current, ANIMATION_CONFIG.SHOW.from, {
    ...
    >
      {shouldRenderTooltip && (
        <div
          ...
          className={cn(
            "absolute inset-0 h-max w-[calc(100%_+_2rem)] flex text-xs flex-col items-center justify-center rounded-xl z-50 shadow-xl px-4 pt-2.5 pb-2 overflow-hidden",
        ...
        >
          ...
          <div className="font-medium text-foreground dark:text-background relative z-30 text-base flex items-center leading-none">
            {mains}
            {subs && (
      ...
      <div ref={contentRef} className={className} onMouseMove={handleMouseMove}>
        {Children}
      </div>
    </div>
  );
