...
const WorksCards = () => {
  ...
  const tryInit = useCallback(() => {
    if (initializedRef.current) return;
    if (!fontsReadyRef.current || !inViewRef.current) return;
    ...
    // Build the loop using the actual elements (safer than global selector)
    loopRef.current = horizontalLoop(cardRefs.current, {
      repeat: -1,
      speed: 1.5,
    });
